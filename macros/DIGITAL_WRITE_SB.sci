...
function [x, y, typ]=DIGITAL_WRITE_SB(job, arg1, arg2)
    ...
    y=[];
    typ=[];
    style = "DIGITAL_WRITE_SB;blockWithLabel;verticalLabelPosition=bottom;" + ..
            "verticalAlign=top;spacing=-2;align=center;"+..
            "displayedLabel=Digital WRITE<br>Pin %s on board %s"
    select job
    case 'plot' then
        // deprecated
    case 'getinputs' then
    ...
    case 'set' then
        ...
        while %t do
            ...
            if num_arduino>2 then
                mess=[mess ;gettext("Only 2 cards  can be used with this toolbox version ")];
                ok=%f;
            end
